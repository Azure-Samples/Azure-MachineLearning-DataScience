...
###########################################
# Binarizer
joinedDF2 <- joinedDF %>% ft_string_indexer(input_col = 'payment_type', output_col = 'payment_ind') %>% 
...
# CREATE TRAIN/TEST PARTITIONS
###########################################
partitions <- joinedDF2 %>% sdf_partition(training = 0.7, test = 0.2, seed = 123)
head(joinedDF2)
```
<hr>
...
fit <- partitions$training %>% 
          ml_random_forest(response = "tip_amount",
                           features = c("pay_type_bin", "fare_amount", "pickup_hour", 
                                        "passenger_count",  "trip_distance", "TrafficTimeBins"), 
