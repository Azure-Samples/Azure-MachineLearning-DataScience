...
This extension enhances the Linux data science VM with  [Jupyterhub](http://jupyterhub.readthedocs.io/en/latest/) that provides a multi user capabilities to Jupyter notebook service. This capability can be useful in training or team environment where a shared VM is used serve multiple users. The extension works by executing the script ```installjupyterhub.sh``` on the VM as a post provisioning step. 

### Accessing Jupyter 
You can access the multiuser jupyter notebook server by visiting https://<VM DNS name>:8000/. But before that, you have to create local user accounts and password on the VM. In Jupyter, use the new OS user login credentials. Once successfully logged in you will see your Jupyter home page with a list of sample notebooks.  
